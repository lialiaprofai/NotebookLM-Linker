...
*   🔑 **Гибкое управление ключами:** Поддержка как серверных API-ключей, так и динамического ввода личных ключей Gemini/Perplexity прямо из веб-интерфейса (с сохранением в `localStorage`).
...
*   Проект разработан с учетом требований безопасности. Все персональные ключи (Gemini/Perplexity API), которые вы вводите во вкладке **Settings** (Настройки), сохраняются исключительно в вашем браузере (`localStorage`) и отправляются в заголовках запросов без сохранения на сервере.
